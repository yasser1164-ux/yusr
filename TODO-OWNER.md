...
**تم ✅ — كل الصور الأساسية جاهزة.** الهيرو، **كل الأقسام الستة**، و**كل المنتجات الثمانية عشر**. كل صورة محفوظة بصيغتين: `.jpg` و `.webp` (المتصفح يختار الأخف تلقائيًا). لم يعد الموقع يعرض أي رسم توضيحي — كل قطعة بصورتها الحقيقية.
**Done ✅ — all core photography is in.** Hero, **all six category tiles**, and **all eighteen products**. Each image is stored twice, `.jpg` + `.webp`; the browser picks the lighter one. The site no longer shows any illustration — every piece has its real photo.
